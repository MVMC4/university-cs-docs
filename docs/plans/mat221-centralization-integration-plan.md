...
**Status:** Phase 0 complete on `feat/mat221-centralization`; architecture gate awaiting review; no MAT221 runtime integration code has been started
...
- every planned page is represented in the tree;
...
1. create `feat/mat221-centralization` from the current updated university `main`;
2. execute Phase 0 only;
3. commit the reproducible inventory, route map, asset manifest, placeholder archive, and visual baseline record;
4. review the architecture gate before making theme, timer, or course-content changes.

Implementation-start note: steps 1–3 are the active scope. Phase 1 does not begin until the Phase 0 records and validation results have been reviewed on the feature branch.

This sequencing is mandatory because the global theme, timer ownership, Fumadocs content structure, and academic-authoring policy affect the whole repository. The Substitution pilot begins only after those contracts are documented and approved.
